...
- **Implementation correction (2026-06-20): `move_mouse_relative` uses an ABSOLUTE `SendInput` + a `SetCursorPos` snap, not the relative move shown in Task 2's code block.** Physical testing proved relative `MOUSEEVENTF_MOVE` deltas are pointer-acceleration "mickeys", not pixels — a 5px request moved **0px**. The shipped `win32_input.py` computes `target = current + delta`, does an absolute `SendInput` (`MOVE | ABSOLUTE | VIRTUALDESK`, normalized `0..65535`, guarded against GetSystemMetrics failure, clamped to `[0,65535]`) to reset the idle timer with real input, then `SetCursorPos(target)` to land on the exact pixel (the `65535`-grid rounding alone drifts ~1–3px per cycle). Same `move_mouse_relative(dx, dy) -> bool` contract, so the controller/GUI are unchanged. See spec §5 and `win32_input.py`. The Task 2 relative code below is retained for historical accuracy only.
- The `SetCursorPos` snap makes each step pixel-exact, so a full 4-step cycle returns to the origin exactly (verified by `test_full_cycle_returns_to_origin_exactly`). Only at the extreme virtual-desktop edge is the absolute target clamped; `sync_position` keeps idle detection correct regardless.
