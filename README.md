...
through Up → Right → Down → Left so there is no net drift over a full cycle.
Before each nudge, if the cursor is within 5 px of any edge of its current monitor,
it is first moved to that monitor's center so the nudge cannot be clamped at the screen boundary.
It also sets `SetThreadExecutionState` while active as a safety net. Each nudge and each
...
- `win32_input.py` — thin `ctypes` wrappers: `GetCursorPos`, `SendInput`,
  `MonitorFromPoint`/`GetMonitorInfo` (edge recenter), `SetThreadExecutionState`.
